...
#' | active | logical scalar | If true, the webhook will send events - if false it is considered inactive and will not generate events. |
#' | resource | list | A generic Asana Resource, containing a globally unique identifier. |
#' | target | character scalar | The URL to receive the HTTP POST. |
#' | created_at | datetime scalar | The time at which this resource was created. |
#' | filters | list | Whitelist of filters to apply to events from this webhook. If a webhook event passes any of the filters the event will be delivered; otherwise no event will be sent to the receiving server. |
#' | last_failure_at | datetime scalar | The timestamp when the webhook last received an error when sending an event to the target. |
#' | last_failure_content | character scalar | The contents of the last error response sent to the webhook when attempting to deliver events to the target. |
#' | last_success_at | datetime scalar | The timestamp when the webhook last successfully sent an event to the target. |
#'
#' @keywords internal
